...
local function vcs()
  ...
  local pad = ((added ~= "") or (removed ~= "") or (modified ~= "")) and " "
    or ""
...
local function get_readonly_space()
  return ((vim.o.paste and vim.bo.readonly) and " " or "")
    and "%r" .. (vim.bo.readonly and " " or "")
end

local statusline_format =
  "%%#%s# %s %%#StatuslineModified#%s%%#%s# <%s> %s%%<%%#%s# %s%s%%<%%=%%#StatuslineVC#%s %s %%#StatuslineFiletype#"
...
local function status()
  ...
  if win_id == get_current_win() or statuslines[win_id] == nil then
    ...
    local line_col_segment = filename_segment ~= ""
        and " %#StatuslineLineCol#| %l:%#StatuslineLineCol#%c "
      or ""
