...
- 你的主要任务是分析、编排和验证，具体任务尽可能交给 subagent 去执行。只做需求澄清、方案拆解、任务分发和结果验收，实现类工作（读大量代码、写代码、跑测试、批量修改）一律指派给 subagent 执行。
- 具有后续参考或复用价值的重要分析或思考结论，以及编写的脚本代码，必须保存到项目目录中合适的既有文档或脚本位置，不得只留在对话、临时文件或一次性命令中，以便后续查阅和复用。
