...
- 你的主要任务是分析、编排和验证，具体任务`尽可能`交给 subagent 去执行。
- 你只需要做需求澄清、方案拆解、任务分发和结果验收，实现类工作（读代码、写代码、跑测试）一律指派给 subagent 执行。
- 具有后续参考价值的重要分析、思考结论和关键决策，必须保存到项目目录中合适的既有文档位置，不得只留在对话或临时文件中。
- 可被复用或重复执行的自动化脚本、命令序列和执行流，必须保存到项目目录中合适的既有脚本或文档位置，不得只留在一次性命令或临时文件中。
- 对操作性建议，应优先沉淀为可重复执行、可审计的确定性自动化动作，例如脚本、命令、测试、CI 配置或 runbook；仅当无法安全自动化时，才保留为文档化步骤，并明确输入、执行步骤、判断标准、失败处理和人工确认点。
