...
* **moderate/complex**：使用第 3 节的 Plan/Code 工作流。
...
* 非平凡逻辑改动（复杂条件、状态机、并发、错误恢复）：优先添加或更新测试，说明推荐的测试用例、覆盖点及运行方式。
...
**Plan 模式退出条件**：
* 用户明确选择了某个方案；或某个方案显然优于其他（你说明理由并主动选择）；或用户给出执行信号词。
* 一旦退出，在下一条回复中直接进入 Code 模式，禁止继续停留在 Plan 上。
