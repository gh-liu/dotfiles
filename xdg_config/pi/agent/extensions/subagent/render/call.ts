...
import { boundedLines, positiveSafeRuntimeIndex, publicRef, renderThinkingLevel, taskTitle, type SubagentRenderArgs, type SubagentRenderContext } from "./shared.ts";
...
export function renderSubagentCall(args: SubagentRenderArgs, theme: Theme, context?: SubagentRenderContext): Text | Box {
  if (args.action === "get") {
    ...
    return renderCallBlock(theme.fg("accent", `◷ get · ${args.ref ? ref ?? "resolving session…" : "sessions"}${args.waitMs ? ` · wait ${Math.ceil(args.waitMs / 1000)}s` : ""}`));
  }
  if (args.action === "cancel" || args.action === "close") {
    const ref = context?.state.ref ?? publicRef(args.ref);
    const color = args.action === "cancel" ? "warning" : "muted";
    return renderCallBlock(theme.fg(color, `${args.action === "cancel" ? "■" : "×"} ${args.action} · ${ref ?? "resolving session…"}`));
  }
  ...
  return renderCallBlock(text);
}

function renderCallBlock(text: string): Box {
