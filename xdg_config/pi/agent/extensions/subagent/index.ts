...
export function registerSubagentExtension(pi: ExtensionAPI, options: SubagentExtensionOptions = {}): void {
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      ...
      "Every subagent task must be self-contained because the child has fresh context: state the outcome, scope and starting evidence, relevant user decisions, constraints and non-goals, acceptance criteria, validation, and exact handoff needed. Do not delegate synthesis the parent has not done.",
      ...
      "Treat a subagent result as a handoff, not proof. A successful read-only handoff with concrete citations is sufficient for synthesis: do not repeat the same searches or re-read every cited file; verify only the few decision-critical, contradictory, or uncertain claims. For write-capable work, inspect the complete scoped diff and rerun relevant integrated validation. Produce the final synthesis yourself.",
