...
export function registerSubagentExtension(pi: ExtensionAPI, options: SubagentExtensionOptions = {}): void {
  ...
  pi.registerTool({
    ...
    promptSnippet: "MANDATORY BEFORE reading target: independent reviews->reviewer, multi-source research->researcher; also delegate multi-file discovery->scout, batch edits/test runs->worker; keep only single-file lookups/edits direct; NEVER call list",
    promptGuidelines: [
      ...
      "Treat a subagent result as a handoff, not proof: for read-only cited work do not repeat the same searches or re-read every file — verify only decision-critical uncertainty; write-capable work requires inspecting the complete settled diff and rerunning relevant integrated validation; when a subagent did not complete normally (failed/crashed/interrupted), the parent must call subagent status to inspect the active/last-settled operation, error, and transcript, and if transcript.sessionPath is present read that session file to locate the last completed step, failure point, and remaining work before deciding to retry or continue; produce the final synthesis yourself.",
    ],
