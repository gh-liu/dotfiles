...
export function registerSubagentExtension(pi: ExtensionAPI, options: SubagentExtensionOptions = {}): void {
  ...
  pi.registerTool({
    ...
    label: "Subagent",
    description:
      `Run a bounded task in a registered subagent session, continue that same context with followup, inspect it with get, stop its active turn with cancel, or release it with close. Use subagents only when specialization, independent judgment, context isolation, or parallel execution provides a concrete benefit. Parallel work is expressed as parallel run calls; the parent owns sequencing and synthesis. Idle sessions keep context but consume no execution slot. At most ${hub.maxConcurrentRuns} turns execute concurrently.\n\nstartup catalog:\n${startupCatalog}`,
    promptSnippet: wakeSnippet,
    promptGuidelines: [
      "Give the child one self-contained task with the necessary paths, evidence, constraints, and expected result in plain text. Do not forward the raw user prompt or add boilerplate fields.",
      ...
      `Use run to create a session and followup on its #N ref when the same agent should refine, fix, or re-check its work. Use parallel run calls for independent work. Runs have no execution deadline; cancel stops only the active turn and close releases the session. Set background only when the parent can continue independently. At most ${hub.maxConcurrentRuns} turns execute at once.`,
      "Treat results as handoffs, not proof. Inspect writing agents' settled changes and run integrated validation. Close sessions when no further followup is useful. Produce the final synthesis yourself.",
    ],
