...
- `close` interrupts active work if necessary, destroys the controller, removes live UI, releases the session, and returns only its ref, agent, and terminal state. It is idempotent for a resolved session. Close is bounded by a single 5-second deadline covering controller readiness, interruption, and controller disposal; on timeout the session reports `crashed`, the slot is released, and disposal continues best-effort in the background.
