...
Tool invocation rows are durable records. Collapsed rows show the actual agent or action plus task title; expanded rows show the bounded task. A followup uses a `↳` continuation marker, resolves the session's registered agent rather than a generic action label, and shows its turn once authoritative progress arrives.
