...
Tool invocation rows are durable records. Collapsed rows show the actual agent, full `provider/model`, and task title; expanded rows show the bounded task without repeating model metadata. A followup uses a `↳` continuation marker, resolves the session's registered agent rather than a generic action label, and shows its turn once authoritative progress arrives.
