...
Foreground and background turns have separate visual owners. A foreground tool result uses a status-colored header for the invocation and current state while keeping the potentially long activity history on the terminal background. The live status line is status-only (`● running`): it never repeats `details.activity`, because the active activity row is the single visual owner of the current tool. The activity region renders earlier-count, completed thinking, completed/failed tools, and active tools at one consistent indentation level; indentation never implies a hierarchy between lifecycle entries. Raw reasoning, tool-call IDs, and tool output are never rendered.
