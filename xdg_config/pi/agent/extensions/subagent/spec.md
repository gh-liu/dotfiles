...
Progress summaries, completion cards, errors, and model handoffs have hard text/envelope bounds. Partial tool rows render an operation-local lifecycle snapshot: up to eight completed/failed calls (`✓`/`✗`) plus current calls with a spinner, with an earlier-call count when truncated. Tool-call IDs deduplicate updates; tool completion moves the item from active to history. Tool summaries are redacted, single-line, and bounded (including a command preview for `bash`); model text/reasoning is never tool history. This renderer-only history is isolated per tool row, works for foreground/background updates, and is discarded in favor of the terminal handoff. Countdown ownership belongs exclusively to the live widget, not partial tool rows.

Activity is also surfaced as a bounded, event-ordered `timeline` for the same renderer/update observability. It is an ordered array interleaving tool calls and thinking segments in the order they occur: tool entries carry `{kind:"tool", id, summary, status}` (appended when the call completes, `status` `completed`/`failed`), and thinking entries carry `{kind:"thinking", text}`. Consecutive thinking deltas merge into a single segment that is flushed only at a boundary (next tool execution or message end), preserving ordering without an entry per delta. The live timeline keeps only the newest 24 entries; both partial and final views render only the newest slice (12 expanded, 8 collapsed) and report an earlier-events count when truncated. Thinking segments are kept in the timeline only for event ordering/observability; the renderer shows a generic `Thinking…` marker (or nothing when a segment carries no text) instead of the reasoning text, so raw reasoning never reaches the UI. Tool summaries remain redacted, single-line, and bounded. The timeline obeys the same safety boundary as the progress rows: it never crosses the model-facing boundary as authoritative content, is renderer/update observability only, and is discarded in favor of the terminal handoff, which never depends on it. Public job payloads use canonical `jobId` plus session-local `ref`; internal operation, process, revision, run, and bare index fields never cross that boundary. Renderer-only details may carry `displayIndex` and `toolProgress`, but model content uses `ref` rather than a bare index.

An optional bounded decision request (`needsDecision: true` plus `decision {question, options?}`) is forwarded from progress into live update `details` only when the child explicitly signals it with a non-empty `question`; the question is redacted/bounded like other progress text, options are trimmed, bounded, and capped at eight, and an all-invalid or absent option list emits no `options` key. Empty or invalid payloads never pollute the public details. M2 scope note: decision *transport* is implemented; the rest of M2 remains partial — decision *handling* (fork, pause/steer, permission/review flows) is not implemented, so a decision request is currently renderer/update observability only. usage/timing accounting is explicitly out of scope / not required and is not a claimed, promised, or pending M2 item.
