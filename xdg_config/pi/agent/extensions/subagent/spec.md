...
The above-editor activity center owns only accepted background turns after their tool call returns. It is a compact session dashboard headed by running/ready counts. Rows are keyed internally by operation and display the session's `#N`, agent, monotonic `turn N`, elapsed time, one bounded task line, and only the latest sanitized activity. It shows at most four sessions; recovery failures and ready results sort before active work, and overflow points to `get`. A settled operation remains `ready · get #N` until Pi emits `message_start` for that exact completion card; queuing `sendMessage` is not treated as delivery acknowledgement. Delivery failure leaves a recoverable `delivery failed · get #N` row. Narrow layouts retain identity, task, and recovery instructions while dropping the activity line below 40 columns.
...
Foreground settlement returns a result-first handoff in the tool result. Its status line shows only the outcome, turn number, and duration because the adjacent invocation row already owns the session ref and agent identity; the actionable footer says `follow up #N or close #N`. Background completion cards have no adjacent invocation row, so their header shows `#N`, agent, turn, outcome, and duration exactly once. Both expose reuse actions only when the runtime is truly idle; closed or crashed workstreams are unavailable, and a crashed result suggests starting a new role session. A card acknowledgement removes only its operation row, never another turn sharing the same public ref.
